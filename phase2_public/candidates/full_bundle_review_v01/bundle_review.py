...
from document_ingestor import write_document_outputs  # noqa: E402
...
def intake(manifest: dict[str, Any], output_root: Path) -> list[dict[str, Any]]:
    ...
    for declared in manifest["documents"]:
        ...
        # Table cells are the review units; the table parent is only a display block.
        blocks = [r for r in locators if r.get("block_type") != "table" and str(r.get("text", "")).strip()]
        documents.append({
            **row,
...
def pair_score(requirement: str, response: str) -> float:
    ...
    if req_refs and req_refs & bid_refs:
        overlap += 0.35
    return round(min(overlap, 1.0), 4)


def find_pair(requirement: dict[str, Any], bid_blocks: list[dict[str, Any]]) -> dict[str, Any]:
    ranked = sorted(
        ((pair_score(requirement["quote"], row["quote"]), row) for row in bid_blocks),
        key=lambda x: (-x[0], x[1]["document_key"], x[1]["block_id"]),
    )
        ...
        for block in doc["blocks"]:
            ...
            if doc["role"] == "tender" and TENDER_CUES.search(text):
                ...
                for task in ("tender_clause_legality", "bid_responsiveness"):
                    pair = find_pair(source, bid_blocks) if task == "bid_responsiveness" else None
                    issues.append({

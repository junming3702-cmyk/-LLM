...
def audit_basis(runtime, finding, usable_evidence, *, locator_spans=None, question_intent=None, citation_audits=None):
    ...
    if locator_spans is not None:
        # Candidate-only: caller constructs spans from declared input locators.
        # Historical v1 behaviour remains unchanged when this is omitted.
        marked_spans = locator_spans
        locators = set(locator_spans)
    ...
    factual = bool(re.search(r"(?:是否实际|是否真实|核实.{0,8}真实性|核验.{0,8}真实性|是否已提交|是否齐全|是否完整提交)", question))
    if question_intent is not None:
        factual = question_intent['has_active_factual_cue']
    if target in TEXT_TARGETS and factual:
    ...
    for check_index, check in enumerate(checks):
        ...
        unbound = len(quote) < 4 or quote not in excerpt or locator not in locators or wrong_span
        if citation_audits is not None:
            unbound = check_index >= len(citation_audits) or not citation_audits[check_index].get('bound')
        if (unbound

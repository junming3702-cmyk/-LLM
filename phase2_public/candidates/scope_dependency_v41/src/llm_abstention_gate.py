...
def _apply_gate(raw_response: Any, runtime_input: dict, *, risk_binding: bool = False, external_scope_bridge: bool = False, source_role_guard: bool = False, external_auto_candidates: bool = False, nu_boundary: bool = False, task_contract_v2: bool = False, scope_dependency_v4: dict | None = None, scope_dependency_v41: dict | None = None, normalize_protocol_v41: bool = False) -> dict:
    ...
    if scope_dependency_v41 is not None:
        ...
        raw_response, protocol_diagnostics = normalize_response(raw_response, enabled=normalize_protocol_v41)
    elif normalize_protocol_v41:
        raise ValueError("v4.1 normalization requires explicit v4.1 scope")
    task_contract_v2 = task_contract_v2 or scope_dependency_v4 is not None or scope_dependency_v41 is not None
    nu_boundary = nu_boundary or task_contract_v2
    actions: list[str] = []
    ...
    for index, finding in enumerate(response["findings"]):
        ...
        original_finding_for_handoff = deepcopy(finding)
        ...
        if nu_boundary:
            if scope_dependency_v41 is not None:
                ...
                def audit_basis(rt, f, usable):
                    return audit_basis_v4(rt, f, usable, scope_dependency_v4)
            elif task_contract_v2:
                from review_task_contract_v2 import audit_basis_v2 as audit_basis
            else:
        ...
        if scope_flags.get("version") == "scope-boundary-v1" and scope_flags.get("task_boundary") is True:
            ...
            if task_contract_v2:
                ...
                ctx = runtime_input.get('project_context') or runtime_input.get('runtime_project_context') or {}
                intent = factual_question_audit(str(ctx.get('review_question') or ''))
            task_boundary, bounded_review = boundary_audit(runtime_input, finding, bounded_review, question_intent=intent)
        ...
        if task_contract_v2:
            if scope_dependency_v41 is not None:
                from scope_dependency_v41 import build_handoff
            elif scope_dependency_v4 is not None:
                from scope_dependency_v4 import build_handoff
            else:
                from review_task_contract_v2 import build_handoff
            _set_field(finding, "review_handoff", build_handoff(original_finding_for_handoff, finding), actions, path)
    ...
    v2_evidence_blocked = bool(task_contract_v2 and any(
        f.get("nu_boundary_audit", {}).get("processing_status") == "evidence_binding_blocked"
        for f in response["findings"]))
    scope_review_blocked = bool(scope_dependency_v41 is not None and any(
        f.get("nu_boundary_audit", {}).get("processing_status") == "scope_review_required"
        for f in response["findings"]))
    technical_blocked = nu_schema_blocked or v2_evidence_blocked
    result = {
        "status": "blocked" if technical_blocked else "scope_review_required" if scope_review_blocked else "corrected" if actions else "passed",
        "blocked": technical_blocked or scope_review_blocked,
        "actions": actions,
        "raw_response": raw_response_preserved,
        "response": response,
    }
    if scope_dependency_v41 is not None:
        result["raw_response"] = v41_original
        result["protocol_diagnostics"] = protocol_diagnostics
        result["technical_blocked"] = technical_blocked
        result["scope_review_required"] = scope_review_blocked
        result["valid_legal_verdicts"] = [
            f.get("conclusion_type") for f in response["findings"]
            if f.get("nu_boundary_audit", {}).get("processing_status") == "valid"]
        result["metric_boundary"] = "Report raw protocol conformance, normalized conformance and delivery status separately; compatibility U is not a legal verdict when blocked."
    return result
...
def apply_gate(raw_response: Any, runtime_input: dict, *, risk_binding: bool = False, external_scope_bridge: bool = False, source_role_guard: bool = False, external_auto_candidates: bool = False, nu_boundary: bool = False, task_contract_v2: bool = False, scope_dependency_v4: dict | None = None, scope_dependency_v41: dict | None = None, normalize_protocol_v41: bool = False) -> dict:
    ...
    normalized, diagnostics = normalize_response(raw_response, enabled=normalize_protocol_v41)
    result = _apply_gate(normalized, runtime_input, **options)
    result["raw_response"] = original
    result["protocol_diagnostics"] = diagnostics
    result.setdefault("scope_review_required", False)
    result.setdefault("technical_blocked", bool(result.get("blocked")))
    result.setdefault("valid_legal_verdicts", [])
    result["metric_boundary"] = "Separate raw protocol conformance, normalized conformance and delivery status. Blocked compatibility labels are not legal verdicts."
    return result

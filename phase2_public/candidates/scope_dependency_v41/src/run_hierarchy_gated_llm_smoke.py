...
def run_final_reasoning(
    *,
    ...
    external_auto_candidates: bool = False,
    nu_boundary: bool = False,
    task_contract_v2: bool = False,
    scope_dependency_v4: dict | None = None,
    scope_dependency_v41: dict | None = None,
    normalize_protocol_v41: bool = False,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Run one final-reasoning pass and apply the deterministic gate."""

    if normalize_protocol_v41 and scope_dependency_v41 is None:
    ...
    if scope_dependency_v41 is not None and scope_dependency_v4 is not None:
        raise ValueError("Select exactly one scope-dependency candidate")
    if scope_dependency_v41 is not None:
        from scope_dependency_v41_prompt import build_prompt
        prompt = build_prompt(prompt)
    elif scope_dependency_v4 is not None:
        from scope_dependency_v4_prompt import build_prompt
        prompt = build_prompt(prompt)
    elif task_contract_v2:
        # This isolated package carries the scope/citation candidate only.
        # The frozen/production package is not changed by this import route.
        from scope_citation_v3_prompt import build_prompt
        prompt = build_prompt(prompt)
    elif nu_boundary:
        from nu_boundary_policy import PROMPT as NU_BOUNDARY_PROMPT
        prompt += NU_BOUNDARY_PROMPT
    ...
    if task_contract_v2:
        ...
        runtime_input["review_task_contract_v2"] = build_contract(runtime_input)
    if scope_dependency_v4 is not None:
        from scope_dependency_v4 import build_contract as build_v4_contract
        runtime_input["review_task_contract_v4"] = build_v4_contract(runtime_input, scope_dependency_v4)
    if scope_dependency_v41 is not None:
        from scope_dependency_v41 import build_contract as build_v41_contract
        runtime_input["review_task_contract_v41"] = build_v41_contract(runtime_input, scope_dependency_v41)
    ...
    if risk_binding or external_scope_bridge or source_role_guard or external_auto_candidates or nu_boundary or task_contract_v2 or scope_dependency_v4 is not None or scope_dependency_v41 is not None:
        gated = apply_gate(raw,runtime_input,risk_binding=risk_binding,external_scope_bridge=external_scope_bridge,source_role_guard=source_role_guard,external_auto_candidates=external_auto_candidates,nu_boundary=nu_boundary,task_contract_v2=task_contract_v2,scope_dependency_v4=scope_dependency_v4,scope_dependency_v41=scope_dependency_v41,normalize_protocol_v41=normalize_protocol_v41)
    else:
        gated = apply_gate(raw,runtime_input)

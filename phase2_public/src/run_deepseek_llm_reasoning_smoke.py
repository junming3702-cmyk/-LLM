...
ENV_FILE = Path(os.environ.get("MODEL_ENV_FILE", MODEL_ROOT / ".env")).expanduser().resolve()
PROMPT_FILE = Path(os.environ.get("SYSTEM_PROMPT_FILE", MODEL_ROOT / "prompts" / "system_prompt_final.md")).expanduser().resolve()
DENSE_RESULT = Path(os.environ.get("DENSE_RESULT_FILE", MODEL_ROOT / ".local_runs" / "retrieval" / "embedding_hybrid_v1" / "dense_comparison_v1.json")).expanduser().resolve()
...
OUT_DIR = Path(os.environ.get("LLM_OUTPUT_DIR", MODEL_ROOT / ".local_runs" / "llm_reasoning_smoke_v1")).expanduser().resolve()
...
def load_api_key() -> str:
    ...
    if not key:
        raise RuntimeError("DEEPSEEK_API_KEY is missing from the process environment or local .env file")
    return key

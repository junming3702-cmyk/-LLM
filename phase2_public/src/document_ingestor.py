"""Document ingestion gate for the Phase 2 contract-review release.
...
Conditional enhancement branches are implemented by separate helpers:
MinerU for inputs already marked ``needs_human_review`` and coordinate OCR as
the backup for unresolved image pages. Neither branch is automatically invoked
by this baseline ingestor or promoted to RAG without quality-gate evidence.
"""
